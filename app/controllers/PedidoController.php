...
class PedidoController extends Controller {
    ...
    public function markItemReady($id_detalle_pedido) {
        $this->requireAuth(['Super Admin', 'Cocina', 'Bar']);
        if ($this->pedidoModel->updateDetallePedidoEstado($id_detalle_pedido, 'Listo')) {
            // Se podría redirigir a la página de comandas o enviar una respuesta JSON
            $_SESSION['success_message'] = 'Item marcado como listo.';
        } else {
            $_SESSION['error_message'] = 'Error al marcar item como listo.';
        }
        // Redirigir según el tipo de comanda o de dónde venga la solicitud
        // Para simplificar, redirigimos a la página principal de pedidos
        $this->redirect('pedidos'); // O más específico: 'pedidos/cocina' o 'pedidos/bar'
    }
